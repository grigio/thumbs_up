  ...
  module ActsAsVoteable #:nodoc:
    ...
    module SingletonMethods
      ...
      def rank_tally(*args)
        ...
        voteable_type = options[:type] || self.base_class.name

        tsub0 = Vote
        tsub0 = tsub0.where("vote = ?", false)
        tsub0 = tsub0.where("voteable_type = ?", voteable_type)
        ...
        tsub1 = tsub1.where("voteable_type = ?", voteable_type)
        ...
        t = t.where("joined_#{Vote.table_name}.voteable_type = '#{voteable_type}'")
      ...
      def tally(*args)
        ...
        t = self.where("#{Vote.table_name}.voteable_type = '#{voteable_type}'")
